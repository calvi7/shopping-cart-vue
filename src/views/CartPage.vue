                ...
                <div class="w-3/4 bg-white px-10 py-10">
                    ...
                    >
                        <div class="flex w-2/5">
                            <div class="w-20">
                                <img :src="product.image" alt class="h-24" />
                            </div>
                        ...
                        <div class="flex justify-center w-1/5">
                            <button @click="cartStore.removeLast(product)">
                            ...
                            <div class="mx-2 border text-center w-8" type="text">{{ product.count }}</div>
                            <button>
                                <svg
...
<script>
import { useCartStore } from "../stores/cartProducts"
...
export default {
    ...
    computed: {
        totalPrice() {
            var sum = 0;
            for (var el in this.cartStore.items) {
                sum += parseFloat(this.cartStore.items[el].price) * parseInt(this.cartStore.items[el].count)
            }
        ...
        cartCount() {
            var sum = 0;
            for (var el in this.cartStore.items) {
                sum += parseInt(this.cartStore.items[el].count)
            }
            return sum
        },
    },
    methods: {
        productPrice(x) {
            var sum = 0
            for (var i = 0; i < this.cartStore.items.length; i++) {
                if (this.cartStore.items[i].id == x.id) {
                    sum += parseFloat(this.cartStore.items[i].price) * parseInt(this.cartStore.items[i].count)
                }
            }
